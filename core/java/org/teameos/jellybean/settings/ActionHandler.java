...
public abstract class ActionHandler {
    ...
    public void performTask(String action) {
        if (action.equals(EOSConstants.SYSTEMUI_TASK_KILL_PROCESS)) {
            killProcess();
        } else if (action.equals(EOSConstants.SYSTEMUI_TASK_SCREENSHOT)) {
            takeScreenshot();
        } else if (action.equals(EOSConstants.SYSTEMUI_TASK_SCREENOFF)) {
            screenOff();
